...
if (error.size < n_resolutions):
	...
	plt.savefig("error_vs_N_satur.png")
...
if max(times) > 0:
	...
	if (error.size < n_resolutions):
		plt.savefig("error_vs_walltime.png")
		plt.loglog(times, error[-1]*times**rate/(times[-1]**rate), '--', label='$\Delta t^{%0.2f}$' % rate)
	else :
		plt.savefig("error_vs_walltime_satur.png")
